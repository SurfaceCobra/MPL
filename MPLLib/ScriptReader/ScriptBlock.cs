...
namespace MPLLib
{
    namespace DataExecute
    {
        public interface ScriptBlock : IRangeInside
        {
            ...
            public static ScriptBlock.Block CreateFixed(IEnumerable<Ranged<string>> reader) => Fix(Create(reader.Append(new Ranged<string>("\x00", int.MaxValue..int.MaxValue)).GetEnumerator(), Bracket.Shape.EOF));
            public static ScriptBlock.Block Create(IEnumerable<Ranged<string>> reader) => Create(reader.Append(new Ranged<string>("\x00", int.MaxValue..int.MaxValue)).GetEnumerator(), Bracket.Shape.EOF);
            private static ScriptBlock.Block Create(IEnumerator<Ranged<string>> reader, Bracket.Shape opener)
            {
                        ...
                        if (Bracket.Groups.TryGetValue(str.o[0], out (Bracket.Shape shape, Bracket.Location loc) val))
                        {
                            ...
                            switch (val.shape, val.loc)
                            {
                                ...
                                case (_, Bracket.Location.Open):
                                    IsNormalText = false;
                                    block.values.Add(Create( reader, val.shape));
                                    break;
